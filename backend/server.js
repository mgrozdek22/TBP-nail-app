...
app.post("/lokacije", async (req, res) => {
  try {
    ...
    if (err.message?.includes("Lokacija se preklapa")) {
  return res.status(409).json({
    ok: false,
...
if (err.message?.includes("Dostupnost se preklapa")) {
  return res.status(409).json({
    ok: false,
...
app.get("/tehnicarke/:id/odobrene-tehnike", async (req, res) => {
  try {
    const tehnicarka_id = toInt(req.params.id);
    const r = await pool.query(
      `SELECT tehnika_id, naziv
       FROM view_tehnicarka_odobrene_tehnike
       WHERE tehnicarka_id = $1
       ORDER BY naziv ASC`,
      [tehnicarka_id]
    );
...
app.get("/tehnicarke/:id/odobreni-oblici", async (req, res) => {
  try {
    const tehnicarka_id = toInt(req.params.id);
    const r = await pool.query(
      `SELECT oblik_id, naziv
       FROM view_tehnicarka_odobreni_oblici
       WHERE tehnicarka_id = $1
       ORDER BY naziv ASC`,
      [tehnicarka_id]
    );
